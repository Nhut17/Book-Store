...
// Change password
export const changePassword = createAsyncThunk('user/changePassword', async (email, thunkAPI) => {
    try {
        const token = localStorage.getItem('token')
        const headers = {
            Authorization: 'Bearer ' + token,
        ...
        console.log(data)
        const res = await axios.get('http://localhost:8083/recoveryPassword/checkOtp',data, {
            headers: headers
        })
        console.log('done')
